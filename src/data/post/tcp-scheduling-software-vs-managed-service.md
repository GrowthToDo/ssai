...
**Direct platform control.** Some nurse managers prefer hands-on involvement in every scheduling decision. With TCP, the nurse manager sees every constraint, makes every assignment, and has full visibility into the scheduling logic. A managed service shifts some of that visibility to the specialist.
...
- Staff self-service features (nurses viewing schedules and requesting swaps via mobile app) are important
...
- Your nurse manager is already stretched thin and cannot absorb a platform learning curve
...
Yes. TCP acquired Aladtec in 2021. The platform continues to operate under the Aladtec interface for most existing customers, but new sales and documentation use the TCP Scheduling Software brand. The product is the same.

**What does TCP Scheduling Software cost for a small hospital?**

TCP pricing is not publicly listed and varies by organization size and contract terms. Mid-market scheduling platforms in this category typically run $200-$600 per month for facilities under 50 beds. Contact TCP directly for current pricing.

**How long does it take to implement TCP Scheduling Software at a CAH?**

Implementation typically runs 2-4 weeks from contract to first operational schedule, assuming the nurse manager can dedicate 5-10 hours to configuration. Facilities with complex staffing rules or large rosters may take longer.
